    ...
    const transformer: ts.Transformer<ts.SourceFile> = (sf: ts.SourceFile) => {
      ...
      function checkNodeForDecorators(node: ts.Node): void {
        ...
        if (isCtorParamsAssignmentExpression(exprStmt)) {
          ...
          iife &&
          isIvyPrivateCallExpression(iife, 'ɵsetClassMetadata')
        ) {
          nodes.push(node);
        } else if (
          iife &&
          ...
          nodes.push(
            ...pickDecorationNodesToRemove(exprStmt, ngMetadata, checker),
          );
        } else if (
          isDecorateAssignmentExpression(exprStmt, tslibImports, checker) ||
          isAngularDecoratorExpression(
            exprStmt,
            ngMetadata,
            tslibImports,
            checker,
          ...
          nodes.push(
            ...pickPropDecorationNodesToRemove(exprStmt, ngMetadata, checker),
          );
        }
      }
...
export function expect<T extends ts.Node>(
  node: ts.Node,
  kind: ts.SyntaxKind,
): T {
  if (node.kind !== kind) {
...
function isAngularCoreImport(
  ...
  // Imports to `@disco3/core` are always core imports.
  if (importText === '@disco3/core') {
    return true;
  }
...
function isDecoratorAssignmentExpression(
...
function isDecorateAssignmentExpression(
  ...
  if (ts.isCallExpression(expr.right)) {
    ...
    if (
      !ts.isIdentifier(innerExpr.left) ||
      !ts.isCallExpression(innerExpr.right)
    ) {
      return false;
    }
...
function isAngularDecoratorExpression(
  ...
  if (
    decorateArray.elements.length === 0 ||
    !ts.isCallExpression(decorateArray.elements[0])
  ) {
    return false;
  }
  ...
  return decorateArray.elements.some((decoratorCall) => {
    if (
      !ts.isCallExpression(decoratorCall) ||
      !ts.isIdentifier(decoratorCall.expression)
    ) {
      return false;
    }
...
function isPropDecoratorAssignmentExpression(
  exprStmt: ts.ExpressionStatement,
): boolean {
...
function isCtorParamsAssignmentExpression(
  exprStmt: ts.ExpressionStatement,
): boolean {
...
function isAssignmentExpressionTo(
  exprStmt: ts.ExpressionStatement,
  name: string,
) {
  if (!ts.isBinaryExpression(exprStmt.expression)) {
...
function getIifeStatement(
  exprStmt: ts.ExpressionStatement,
): null | ts.ExpressionStatement {
...
function pickDecorationNodesToRemove(
  ...
  const expr = expect<ts.BinaryExpression>(
    exprStmt.expression,
  ...
  const ngDecorators = elements.filter((elem) =>
    isAngularDecorator(elem, ngMetadata, checker),
  );
...
function pickDecorateNodesToRemove(
  ...
  if (ts.isCallExpression(exprStmt.expression)) {
    ...
    if (ts.isCallExpression(expr.right)) {
      ...
      ts.isCallExpression(expr.right.right)
    ) {
  ...
  const callCount =
    ngDecoratorCalls.length + metadataCalls.length + paramCalls.length;
...
function isAngularDecorator(
  ...
  const assign = expect<ts.PropertyAssignment>(
    types[0],
...
function isTslibHelper(
  ...
  if (
    ...
    callExpr.expression.text !== helper
  ) {
    return false;
  }
  ...
  for (const dec of symbol.declarations) {
    if (
      ...
      tslibImports.some((name) => name.elements.includes(dec))
    ) {
      return true;
    }
