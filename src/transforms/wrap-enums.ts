  ...
  return (context: ts.TransformationContext): ts.Transformer<ts.SourceFile> => {
    const transformer: ts.Transformer<ts.SourceFile> = (sf) => {
      ...
      return context.factory.updateSourceFile(
        sf,
        ts.setTextRange(result, sf.statements),
      );
    };
...
function visitBlockStatements(
  ...
  for (
    ...
    if (
      ...
      const variableDeclaration =
        currentStatement.declarationList.declarations[0];
  ...
  return updatedStatements
    ? nodeFactory.createNodeArray(updatedStatements)
    : statements;
}
...
function findEnumIife(
  ...
  for (const statement of functionExpression.body.statements) {
    ...
    if (
      ...
      leftExpression.text !== parameterName
    ) {
      return null;
    }
  }
    ...
    if (ts.isCallExpression(expression)) {
      ...
      if (args.length > 2) {
        const isReferenced = args.some((arg) => {
          const potentialIdentifier = ts.isPropertyAccessExpression(arg)
            ? arg.expression
            : arg;

          return (
            ts.isIdentifier(potentialIdentifier) &&
            potentialIdentifier.text === name
          );
        });
...
function updateEnumIife(
  ...
  const updatedFunction = nodeFactory.updateFunctionExpression(
    ...
    nodeFactory.updateBlock(expression.body, [
      ...
      nodeFactory.createReturnStatement(
        expression.parameters[0].name as ts.Identifier,
      ),
    ]),
  );
  ...
  if (exportAssignment) {
    arg = nodeFactory.createBinaryExpression(
      exportAssignment,
