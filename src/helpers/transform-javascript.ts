...
export function transformJavascript(
  ...
  const parseDiagnostics = (tempSourceFile as DiagnosticSourceFile)
    .parseDiagnostics;
  ...
  if (allowFastPath && parseDiagnostics) {
    ...
    const transforms = getTransforms.map((getTf) =>
      getTf(/* program */ undefined),
    );

    const result = ts.transform(tempSourceFile, transforms, tsOptions);
    if (
      result.transformed.length === 0 ||
      result.transformed[0] === tempSourceFile
    ) {
      return {
        content: null,
        sourceMap: null,
